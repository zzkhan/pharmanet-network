...
func (s *SmartContract) getDrugCRPs(ctx contractapi.TransactionContextInterface, collection string, drugId string) (*DrugCrps, error) {
	crpBytes, err := ctx.GetStub().GetPrivateData(collection, drugId)
	if err != nil {
		return nil, fmt.Errorf("failed to read crps for drug %s from collection: %s %v", drugId, collection, err)
	}
	var crps DrugCrps
	err = json.Unmarshal(crpBytes, &crps)
	if err != nil {
		return nil, err
	}
	return &crps, nil
...
func createNewDrugCrps(crpMapData map[string]string) (DrugCrps, error) {

	var crpList []Crp
	for key, value := range crpMapData {
		fmt.Printf("Key: %s, Value: %s\n", key, value)
		crpList = append(crpList, Crp{
			Challenge: key,
			Response:  value,
			Assignee:  "",
			Used:      false,
...
func (s *SmartContract) verifyDrugDoesNotExist(ctx contractapi.TransactionContextInterface, drugId string) error {
	exists, err := s.AssetExists(ctx, drugId)
	if err != nil {
